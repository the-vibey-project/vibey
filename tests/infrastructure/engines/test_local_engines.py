...
"""The one local-engine resolver, and the one local endpoint setting (ADR-0038, ADR-0064)."""
...
def test_only_gptossloop_is_on_by_default() -> None:
    """ADR-0064: the sovereign default engine ships on; every other local engine is opt-in."""
    settings = _settings()
...
def test_a_qwenloop_switch_is_told_what_it_now_means() -> None:
    """ADR-0064: an operator who switched qwenloop on for gpt-oss hears it is Qwen now."""
...
def test_vibey_ollama_url_becomes_each_runners_openai_compat_endpoint() -> None:
    ...
    endpoint, and runs the Qwen model it names itself (ADR-0064)."""
