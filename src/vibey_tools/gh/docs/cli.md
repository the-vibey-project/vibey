...
| `fit` | optional `--model`, `--queue`, `--payload-bytes`, `--deadline`, `--observed-seconds`, `--journal` | The fit calculus: decide whether this machine should take a given piece of work, from the model actually wanted, the queue already ahead of it, the payload size and the caller's deadline. `--observed-seconds` records what a payload really took and `--journal` persists the decision and reads prior ones back, so repeated calls form a self-adjusting estimate rather than a fixed guess. The full model is documented under [The fit calculus](configuration.md#the-fit-calculus--vibey-gh-fit). |
