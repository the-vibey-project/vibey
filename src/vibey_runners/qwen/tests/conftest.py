...
def isolated_settings(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    """Keep the operator's own gptossloop and qwenloop settings out of every test.

    The CLI layers `$QWENLOOP_CONFIG` (else the user config dir) and `QWENLOOP_*`
    variables into every command -- `GPTOSSLOOP_*` when it runs as gptossloop -- so a developer with Ollama configured would otherwise
    ...
    for prefix in ("QWENLOOP", "GPTOSSLOOP"):
        for suffix in ("BASE_URL", "MODEL", "API_KEY"):
            monkeypatch.delenv(f"{prefix}_{suffix}", raising=False)
    monkeypatch.delenv("GPTOSSLOOP_CONFIG", raising=False)
    return config
