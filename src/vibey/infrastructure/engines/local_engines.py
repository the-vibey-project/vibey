...
- `LocalEngineSettings` answers from the environment first, then the project's
  ...
  for every other local engine (ADR-0062).
...
#: vibey hands it only the endpoint (ADR-0062).
...
class LocalEngineSwitch:
    ...
    whether the engine is on when neither sets it (ADR-0062)."""
...
#: What an operator who switched qwenloop on before ADR-0062 needs to hear: the switch
#: they set now means something else.
QWENLOOP_SWITCH_NOTICE = (
    "qwenloop is switched on, and since ADR-0062 it runs a Qwen model (qwen3:14b unless "
...
class LocalEngineSettings:
    ...
    def notices(self) -> tuple[str, ...]:
        """What the operator should hear about how their switches now read (ADR-0062)."""
...
class LocalEndpointEnvironment:
    ...
    its own names for them (ADR-0062):
