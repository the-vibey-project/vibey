...
class ClaudeLoopWorkPlanProducer:
    ...
    async def decompose(self, spec: DesignSpec) -> tuple[WorkItem, ...]:
        ...
        # Caught live: the model returned ids like "WI-01" despite the prompt, so ids
        # are normalised on decode -- and two that normalise to one must be refused
        # here, before they become one branch.
