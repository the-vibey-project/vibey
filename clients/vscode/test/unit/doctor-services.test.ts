...
import { CatalogueParser, DegradedCatalogue } from '../../src/core/catalogue';
import { Doctor, type DoctorDependencies } from '../../src/core/doctor';
import { EngineCommand } from '../../src/core/engine-command';
import { ForbiddenEnvironment } from '../../src/core/environment';
import type { Catalogue, CatalogueEngine } from '../../src/core/interfaces/catalogue-interface';
import type { OllamaProbeInterface, OllamaStatus } from '../../src/core/interfaces/ollama-interface';
import type { RunRequest } from '../../src/core/interfaces/run-interface';
import type { RawSettings } from '../../src/core/interfaces/settings-interface';
import type { VolatileHit } from '../../src/core/interfaces/storage-interface';
import { OllamaAdvice } from '../../src/core/ollama';
...
describe('Doctor', () => {
  const doctor = (overrides: Partial<DoctorDependencies> = {}, present = ['git', 'qwenloop', 'vibey', 'vibey-skills'], processes = new FakeProcessRunner()) => {
  ...
  const versions = (): FakeProcessRunner =>
    new FakeProcessRunner()
      ...
      .on(['/opt/bin/qwenloop', '--version'], { stdout: 'qwenloop 0.9.0\n' })
  ...
  it('passes a ready machine, naming each program with where it was found and what version it is', async () => {
    ...
    expect(detail.loops).toBe('vibey loops: sovereignloop (qwenloop); paidloop (claudeloop, codexloop, cursorloop, agyloop)');
  ...
  it('says what is missing or wrong, with steps a beginner can follow', async () => {
    const processes = new FakeProcessRunner()
      .on(['/opt/bin/qwenloop', '--version'], { code: 1, stderr: 'ImportError: no module named qwenloop\n' })
    ...
    expect(byName.get('qwenloop')?.detail).toBe('/opt/bin/qwenloop (from PATH) did not answer --version: ImportError: no module named qwenloop');
...
describe('CoreServices', () => {
  const build = (options: { present?: string[]; raw?: Partial<RawSettings>; platform?: string; environ?: Record<string, string> } = {}) => {
    ...
    const present = new Set((options.present ?? ['git', 'qwenloop', 'vibey', 'vibey-skills']).map((name) => path.join(bin, name)));
    const processes = new FakeProcessRunner();
  ...
  it('reads vibey loops once until asked again, and says plainly when there is no vibey', async () => {
    ...
    const degraded = await build({ present: ['git', 'qwenloop'] }).core.catalogue();
  ...
  it("hands each run its services: the selector, a slot per machine or paid engine, what Ollama holds, and the person's config", async () => {
    ...
    const local = services.lockFor(qwenloop, 'local');
    ...
    const declared = build({ raw: { qwenloopPath: '/opt/q/qwenloop' } }).core;
    expect((await declared.runServices()).command(qwenloop)).toBe(
    ...
    expect(services.userConfig()).toBeUndefined();
    const config = path.join(home, 'qwenloop.toml');
    fs.writeFileSync(config, 'context_window = 8192\n');
    expect((await build({ environ: { QWENLOOP_CONFIG: config } }).core.runServices()).userConfig()).toEqual({ path: config, text: 'context_window = 8192\n' });
  ...
  it('starts a task through the queue, with the real selector and slot, and records it', async () => {
    ...
    expect(record).toMatchObject({ outcome: 'wound-down', engine: 'qwenloop', max_turns: 16 });
    const child = processes.children[0];
    expect(child?.command).toBe(path.join(bin, 'qwenloop'));
