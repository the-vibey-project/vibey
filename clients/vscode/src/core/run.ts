 ...
 * (ADR-0062: gptossloop, the default, and qwenloop), and only for the binding the family
...
export class TaskRun implements TaskRunInterface {
  ...
  private environmentFor(selection: Selection, runner: RunnerIdentity | undefined): Record<string, string> {
    ...
    if (runner !== undefined) {
      ...
      // gets a model only when one is named, else its own config chooses (ADR-0062).
