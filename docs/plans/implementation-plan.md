...
>   ADR-0015, and the sovereign DESIGN provider, ADR-0027; since ADR-0062 that
...
| 5.3 | `design.research` — parallel, `untrusted` provenance, web + docs MCP | — | research output never enters as instruction | **done** — `application/design_research_handler.py`; with `--provider gptossloop` (the default; `qwenloop` before ADR-0062) research reads `$VIBEY_EVIDENCE_DIR` (ADR-0027) |
