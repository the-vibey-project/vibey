...
>
> - There are **six** engine ids over five runners (a seventh, `opencode`, and
>   its runner `opencodeloop` were deleted after canon 8.b repealed OpenCode). `qwenloop` and
...
since been deleted); the latter two have `tier = LOCAL`, which is what the
