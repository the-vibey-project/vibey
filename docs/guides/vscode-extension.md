...
sovereignloop, runs on your machine: qwenloop, vibey's local agent, drives **gpt-oss:20b**
through [Ollama](https://ollama.com). The design behind it is
[ADR-0059](../architecture/decisions/0059-the-editor-drives-the-familys-own-loops.md).
...
- **vibey**, which brings `qwenloop` and `vibey-skills`: `pip install vibey` (Python 3.12+).
  The loop, effort and engine pickers read `vibey loops --json`, which ships in vibey 3.0.0.
  With an older vibey the extension says so, and runs sovereignloop with qwenloop.
