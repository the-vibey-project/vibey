...
`opencodeloop` (plus the local runner's two engines: `gptossloop`, the sovereign
default on GPT-OSS 20B, and opt-in `qwenloop` on Qwen) via PostgreSQL queue with lossless
...
(claudeloop, codexloop, cursorloop, agyloop, opencodeloop, gptossloop and
qwenloop — one package, two engines, ADR-0060 — and vibey-runners-common) and
...
- **Engines:** claudeloop, codexloop, cursorloop, agyloop, opencode — the paid pool,
  ...
  `VIBEY_FEATURE_CLAUDELOOP_LOCAL` or `[features] claudeloop_local` (ADR-0060) —
  ...
  sovereign gptossloop providers (ADR-0027, ADR-0038, ADR-0060). `VIBEY_OLLAMA_URL` is the one local endpoint setting.
...
| ADRs | `docs/architecture/decisions/` (60 ADRs: 0001–0060) |
