...
- Never implement on `main`. PRs squash into `develop` via the merge train
  ...
  `main` (linear history). `develop` → TestPyPI, `main` → PyPI, for `vibey-engine` and `krypton-app` (ADR-0067).
  ADR-0028.
...
names — the tree ships as one `vibey-engine` package (ADR-0037, ADR-0067); the apps ship as `krypton-app`.
...
| ADRs | `docs/architecture/decisions/` (67 ADRs: 0001–0067) |
