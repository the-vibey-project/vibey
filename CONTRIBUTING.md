...
background, the test databases no live session holds (`tests/db_reaper.py`). A database is
kept while its lock is held, or while the process its mark names is alive on this machine, so
a session that loses its lock mid-run still keeps its databases. `uv run python -m tests.db_reaper --dry-run` shows what it would drop.
