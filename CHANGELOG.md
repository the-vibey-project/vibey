...
* **cli:** `vibey loops [--json]` lists the family's two loops (sub-doctrines 8.b and 8.c):
  `sovereignloop` (tier local, the default) and `paidloop` (tier paid, declared-only, with
  claudeloop as its default). For each engine it shows all five efforts: the argv, the effort
  really achieved, and the model or what chooses it. It also gives the escalation ladder and a
  by-effort view. Each engine reports what it can take: images, files, pasted text or images,
  plugins and MCP. A value is `null` where the runner's own code shows nothing, and
  `evidence` names the proof wherever it shows something. Each engine also reports its `run`
  argv template, held to `build_argv` for every engine and effort, and its `stop`,
  `wind_down` and `prompt` verbs, held to each runner's own Typer definition. The report adds
  where its events land, how they are shaped, and the environment names it reads (names
  only). The command needs no database and no network. `EngineDescriptor` gains
  `affordances`, `controls` and `events`, all unknown by default. OpenCode is listed under its
  descriptor's tier, with a note that 8.b repeals it
