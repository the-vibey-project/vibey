...
* **queue:** a job can be bumped to run next (ADR-0054). `vibey queue bump JOB` puts it
  ...
  `migrations/0015_job_bump_named.sql` is additive: it keeps 0014's `bump_origin` column,
  unused, so those workers still read every row; a later contract migration drops it.
