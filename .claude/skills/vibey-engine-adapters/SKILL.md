...
description: How vibey drives claudeloop, codexloop, cursorloop, agyloop, and the local engines — gptossloop (the sovereign default), and the opt-in qwenloop and claudeloop-local — the engine adapter pattern, tiers, argv building, and the conformance suite.
...
`LOCAL_DESCRIPTORS`) join them, each behind its own switch (ADR-0061):
...
`CURSORLOOP`, `AGYLOOP`, `GPTOSSLOOP`, `QWENLOOP`, `CLAUDELOOP_LOCAL` — one
...
`DEFAULT_DESCRIPTORS` is the default pool of four, `LOCAL_DESCRIPTORS` the three
local ones, `ALL_DESCRIPTORS` all seven, and `BY_ENGINE_ID` maps every
`EngineId`. The worker
