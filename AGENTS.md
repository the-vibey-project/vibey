...
- `src/vibey_runners/{claude,codex,cursor,agy,qwen}` — claudeloop,
  ...
  engines — ADR-0062); `src/vibey_runners/common` — vibey-runners-common.
...
- **Engines:** `claudeloop`, `codexloop`, `cursorloop`, and `agyloop` are the
  ...
  claudeloop_local`). ADR-0062. Under sub-doctrine 8.a local engines are
  **preferred first**: BUILD selection runs SWRR within the LOCAL tier and falls
  ...
  ADR-0027, ADR-0038, ADR-0062). `VIBEY_OLLAMA_URL` is the one local endpoint
  setting.
