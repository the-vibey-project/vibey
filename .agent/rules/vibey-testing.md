...
on its database while it lives, and every run drops, in the background, the test databases no
live session holds (`tests/db_reaper.py`). See what would go with
