...
| `src/vibey_runners/opencode` | `opencodeloop` | 3.12 | pytest, ruff, mypy, import-linter |
...
## Convergence-Driven Development is the delivery loop
...
Convergence-Driven Development (CDD) above them to ground, implement, test,
review and repair against the actual tracked repository. At every iteration,
classify the trajectory as converging, neutral or diverging against the
remaining criteria and failing checks. A divergence is allowed only when it is
small, bounded and has a concrete path to reconvergence; large or pathless
divergence is abandoned. Never treat activity, a marker or a generated file as
completion evidence. See sub-doctrine 9.c and ADR-0039.
Project atoms can compose chemical structures, and a suite of suites can be
alive in the digital realm as a software organism when identity, feedback,
adaptation, repair and exchange are evidenced; inspect that higher-order
trajectory whenever the change participates in it.
